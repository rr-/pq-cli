...
class CharacterSheetWindow(DataTableProgressBarWindow):
    def __init__(
    ...
    def _sync_exp(self) -> None:
        cur = self._player.exp_bar.position
        max_ = self._player.exp_bar.max_
        text = f"Experience ({max_-cur:.0f} XP to go)"
