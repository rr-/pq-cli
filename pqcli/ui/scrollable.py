...
class _Scrollable(urwid.WidgetDecoration):
    def __init__(self, widget):
        ...
        possibly not intuitively, depending on the arrangement of widgets.
        When switching focus to a widget that is ouside of the visible part of
        the original widget, the canvas scrolls up/down to the focused widget.
        It would be better to scroll until the next focusable widget is in
        sight first. But for that to work we must somehow obtain a list of
        focusable rows in the original canvas.
        """
    ...
    def render(self, size, focus=False):
        ...
        if canv.cursor is not None:
            ...
            if canv_full.cursor is not None:
                ...
                # ask canv whether it contains bits of the focused widget.
